...
// Layouts das 8 fases — plataformas [x, y, w, h]
const PHASE_LAYOUTS = [
  {
  ...
  },
  {
    ...
    collectibles: [{ x:300,y:315,type:'point' },{ x:480,y:255,type:'point' },{ x:670,y:195,type:'life' }],
    goal:         { x:820, y:370 },
    playerStart:  { x:60, y:370 }
  },
  {
...
export class GameScene extends Phaser.Scene {
  ...
  private score = 0
  private lives = 1          // vidas globais (1-3)
  private scoreAtLastLife = 0 // controla marco de +100pts
  ...
  init(data: GameData) {
    ...
    this.scoreAtLastLife = this.score  // referência para o próximo marco
    this.enemies      = []
    this.collectibles = []
  ...
  create() {
    ...
    // HUD recebe corações, vidas e pontuação
    ...
    // ── Callbacks do Player ───────────────────────────────────────────
    this.player.onHeartChange((hearts) => {
      this.scene.get('HUDScene').events.emit('updateHearts', hearts)
    })
  ...
  private addScore(pts: number, label?: string) {
    ...
    // Texto flutuante
    const displayLabel = label ?? `+${pts}`
    const txt = this.add.text(this.player.x, this.player.y - 35, displayLabel, {
    ...
    // Marco de +100pts → +1 vida (máx 3)
    this.checkScoreMilestone()
  }

  private checkScoreMilestone() {
    const gained = this.score - this.scoreAtLastLife
    if (gained >= 100 && this.lives < 3) {
  ...
  private showBonusLife() {
    const { width, height } = this.cameras.main
    const panel = this.add.graphics().setScrollFactor(0).setDepth(20)
    panel.fillStyle(0x1a0a2e, 0.85)
    panel.fillRoundedRect(width / 2 - 130, height / 2 - 30, 260, 60, 12)

    const txt = this.add.text(width / 2, height / 2, '✨ +1 Vida! ✨', {
      fontSize: '22px', color: '#f5c842', fontStyle: 'bold', fontFamily: 'Arial'
  ...
  private handlePlayerDeath() {
    ...
    this.lives = this.lives - 1   // perde 1 vida ao zerar os 3 corações
    StorageManager.save({ lives: this.lives })

    const { width, height } = this.cameras.main
    const panel = this.add.graphics().setScrollFactor(0).setDepth(20)
    panel.fillStyle(0x1a0a2e, 0.92)
    panel.fillRoundedRect(width / 2 - 160, height / 2 - 60, 320, 120, 14)

    if (this.lives <= 0) {
      // ── GAME OVER ────────────────────────────────────────────────────
      StorageManager.reset()  // reseta o save
      this.add.text(width / 2, height / 2 - 25, '💀 GAME OVER', {
        fontSize: '26px', color: '#e74c3c', fontStyle: 'bold', fontFamily: 'Arial'
      }).setOrigin(0.5).setScrollFactor(0).setDepth(21)

      this.add.text(width / 2, height / 2 + 15, 'Voltando ao menu...', {
      ...
      // ── Perdeu vida mas ainda tem mais ──────────────────────────────
      this.add.text(width / 2, height / 2 - 25, 'Não desista!', {
        fontSize: '22px', color: '#f5c842', fontStyle: 'bold', fontFamily: 'Arial'
      }).setOrigin(0.5).setScrollFactor(0).setDepth(21)

      this.add.text(width / 2, height / 2 + 12, `Vidas restantes: ${'❤️'.repeat(this.lives)}`, {
        fontSize: '16px', color: '#fff8e1', fontFamily: 'Arial'
      }).setOrigin(0.5).setScrollFactor(0).setDepth(21)

      this.time.delayedCall(2000, () => {
        this.scene.stop('HUDScene')
        // Precisa acertar o quiz de novo para entrar na fase
  ...
  private drawBackground() {
    ...
    const hills = this.add.graphics().setScrollFactor(0.3).setDepth(-9)
    ...
    this.drawTree(hills, 120, height - 110)
    this.drawTree(hills, 450, height - 100)
    this.drawTree(hills, 720, height - 120)
  }

  private drawTree(g: Phaser.GameObjects.Graphics, x: number, y: number) {
    g.fillStyle(0x8b5e3c, 1); g.fillRect(x - 4, y, 8, 30)
    g.fillStyle(0x6b8c3c, 1); g.fillCircle(x, y - 15, 22)
    g.fillStyle(0x7a9c40, 0.7)
    g.fillCircle(x - 12, y - 8, 14)
    g.fillCircle(x + 12, y - 10, 14)
  }
  ...
  private spawnEnemies(layout: typeof PHASE_LAYOUTS[0]) {
    layout.enemies.forEach(e => {
      const enemy = new Enemy(this, e.x, 380, e.patrol[0], e.patrol[1], this.world.enemy.speed)
  ...
  private spawnGoal(layout: typeof PHASE_LAYOUTS[0]) {
    ...
    g.fillStyle(0xf5c842, 1); g.fillRect(layout.goal.x, layout.goal.y - 60, 4, 60)
    g.fillStyle(0xe8a020, 1)
    g.fillTriangle(layout.goal.x + 4, layout.goal.y - 60, layout.goal.x + 4, layout.goal.y - 40, layout.goal.x + 28, layout.goal.y - 50)

    const aura = this.add.graphics().setDepth(2)
    aura.fillStyle(0xf5c842, 0.2); aura.fillCircle(layout.goal.x + 4, layout.goal.y - 30, 25)
    this.tweens.add({ targets: aura, alpha: 0.5, scaleX: 1.3, scaleY: 1.3, yoyo: true, repeat: -1, duration: 800 })
  ...
  private setupCollisions() {
    ...
    this.enemies.forEach(enemy => {
      ...
      this.physics.add.overlap(this.player, enemy, () => {
        ...
        if (pb.velocity.y > 0 && this.player.y < enemy.y - 10) {
          enemy.stomp()
          this.addScore(50, '+50 ⭐')
          pb.setVelocityY(-300)
        } else {
      ...
      this.physics.add.overlap(this.player.getProjectiles(), enemy, (proj) => {
        ...
        p.setActive(false); p.setVisible(false)
    ...
    this.collectibles.forEach(col => {
      this.physics.add.overlap(this.player, col, () => {
        ...
        if (col.collectibleType === 'point') {
          // Fragmento de Luz → +1pt
          this.addScore(1, '+1 ✦')
        } else {
          // Coração no mapa
          const restored = this.player.addHeart()
          if (restored) {
            // Coração restaurado
            this.scene.get('HUDScene').events.emit('updateHearts', this.player.getHearts())
            this.showFloatingText('❤️ +1 Coração!', '#ff6b6b')
          } else {
            // Já estava com 3 → +10pts
            this.addScore(10, '+10 ⭐')
          }
        }
      })
  ...
  private showFloatingText(text: string, color = '#f5c842') {
    const txt = this.add.text(this.player.x, this.player.y - 35, text, {
